...
- **Super-admin** – **/superadmin**: list sites, add site (name + domain), edit (domain, name, features), copy site key. Same login as /admin. Access at **https://seashell-camel-446716.hostingersite.com/superadmin** (or **https://asoldi.com/superadmin** when you change domain).
- **Client CMS** – **/admin** fetches `GET /api/cms/config` (lookup by `CMS_SITE_KEY` or by Host). Sidebar shows only Users / Analytics / Ecommerce according to hub. This site is a client: add it in super-admin with domain `seashell-camel-446716.hostingersite.com` (or `asoldi.com`) and optionally set `CMS_SITE_KEY` in env; otherwise config is resolved by domain.
- **Installable** – **@asoldi/client-cms** npm package (from [website-cms](https://github.com/Damianhch/website-cms)): install in any project, mount routes at `/api/cms`, add route for `/admin` with `ClientCMS`. Set `CMS_HUB_URL` and `CMS_SITE_KEY` on the client host; domain can be changed later in the hub without changing the client.
