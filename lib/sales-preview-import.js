...
import { assertImportedPreviewHasAssets } from './preview-bundle-assets.js';
...
export function getPublicSalesPreviewUrl(client = null, { pretty = true } = {}) {
  if (!client?.id) return '';
...
export async function ingestSalesPreviewZip({
  ...
  const publicUrl = `${getPublicPreviewOrigin()}${prettyPath}`;
