...
const SKIP_RECOVER_HOSTS =
  /(^|\.)(asoldi\.com|googleapis\.com|gstatic\.com|google\.com|facebook\.com|twitter\.com|instagram\.com|cloudflare\.com|google-analytics\.com|googletagmanager\.com)$/i;
...
export function collectRecoverableHomepages(html = '') {
  ...
  for (const match of String(html || '').matchAll(/https?:\/\/[^"'()\s<>]+/gi)) {
    try {
      ...
      if (parts[0]) bump(`${parsed.origin}/${parts[0]}/`);
      if (parts[1]) bump(`${parsed.origin}/${parts[0]}/${parts[1]}/`);
    } catch {
...
function stylesheetHrefsFromHtml(html = '', baseUrl = '') {
  ...
  for (const match of String(html || '').matchAll(/<link\b([^>]*?)>/gi)) {
    ...
    try {
      hrefs.push(new URL(href, baseUrl || undefined).toString());
    } catch {
      // skip
    }
  }
...
export async function recoverMissingStylesheets(html = '', { fetchImpl = fetch, timeoutMs = 20_000 } = {}) {
  const source = String(html || '');
  if (!hasLocalStylesheetLinks(source)) return source;
  const homes = collectRecoverableHomepages(source);
  if (!homes.length) return source;
  const chunks = [];
  ...
  stripped = stripped.replace(/<script\b([^>]*?)>\s*<\/script>/gi, (full, attrs) => {
    const src = attrs.match(/\bsrc\s*=\s*["']([^"']+)["']/i)?.[1] || '';
    if (!src || /^(https?:)?\/\//i.test(src) || src.startsWith('data:')) return full;
    return /\.js(\?|#|$)/i.test(src) ? '' : full;
  });
