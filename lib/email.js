...
function readSmtpConfig(prefix = 'SMTP') {
  ...
  const pass = normalizeHeaderValue(process.env[`${prefix}_PASS`]);
...
function buildMessage(payload, prefix = 'SMTP') {
  const { to, subject, text, html, replyTo, attachments, headers, icalEvent } = payload || {};
...
export async function sendEmail({ to, subject, text, html, replyTo, attachments, headers, icalEvent }) {
  const payload = { to, subject, text, html, replyTo, attachments, headers, icalEvent };
