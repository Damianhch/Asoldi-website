...
function thankYouBodyHtml(client, calendar = {}, { mergeTags = false } = {}) {
  ...
  const viaLine = isInPerson(client)
    ...
    : 'Trykk på knappen under for å bli med i Google Meet.';
...
function withLayout(client, calendar, options, view) {
  ...
  const payload = {
    ...
    signerHtml: signerHtml({ mergeTags, sender }),
...
export function buildSalesCalendarInvite(client, calendar = {}, options = {}) {
  ...
  if (!inPerson && !isRealMeetLink(meetLink)) return null;
  ...
  const description = [
    ...
    inPerson ? `Adresse: ${location}` : `Google Meet: ${meetLink}`,
  ...
  const icsLines = [
    ...
    !inPerson && meetLink ? `X-GOOGLE-CONFERENCE:${escapeIcsText(meetLink)}` : '',
    'END:VEVENT',
    'END:VCALENDAR',
  ].filter(Boolean);
