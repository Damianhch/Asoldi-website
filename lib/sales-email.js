...
export const SALES_EMAIL_FROM = 'Asoldi <kontakt@asoldi.com>';
const SIGNER_NAME = 'Damian fra Asoldi';
...
function signerHtml({ mergeTags = false } = {}) {
  const name = mergeOrValue(mergeTags, '{{signerName}}', escapeHtml(SIGNER_NAME));
  const email = mergeOrValue(mergeTags, '{{signerEmail}}', escapeHtml(SIGNER_EMAIL));
  const phone = mergeOrValue(mergeTags, '{{signerPhone}}', escapeHtml(SIGNER_PHONE));
  return `
    <p style="margin:0 0 8px;">${name}</p>
    <p style="margin:0 0 10px;color:#5b5b5b;">P.S. Svar på denne e-posten hvis du må flytte møtet.</p>
    <p style="margin:0;">
      <a href="mailto:${mergeTags ? '{{signerEmail}}' : SIGNER_EMAIL}" style="color:#1a1a1a;text-decoration:none;">${email}</a>
      &nbsp;|&nbsp;
      <a href="tel:+4748339191" style="color:#1a1a1a;text-decoration:none;">${phone}</a>
    </p>`;
}
...
function thankYouBodyHtml(client, calendar = {}, { mergeTags = false } = {}) {
  ...
  const viaLine = isInPerson(client)
    ? `Vi sees på ${mergeOrValue(mergeTags, '{{meetingPlace}}', escapeHtml(sanitize(client?.meetingPlace) || 'avtalt adresse'))}.`
    : 'Du kan bli med i det online møtet via møtelenken over.';
  ...
  return `
    ...
    <p style="margin:0 0 8px;">${viaLine}</p>`;
}

function reminderBodyHtml(client, reminderKind = '24h', { mergeTags = false } = {}) {
  const firstName = mergeOrValue(mergeTags, '{{firstName}}', escapeHtml(extractFirstName(client?.contactPerson)));
  const whenShort = mergeOrValue(mergeTags, '{{meetingWhen}}', escapeHtml(formatMeetingShort(client?.meetingAt)));
  const horizon = reminderKind === '1h' ? 'om 1 time' : 'om 24 timer';
  const extra = reminderKind === '1h'
    ? 'Åpne gjerne 2 minutter før, så vi kommer raskt i gang.'
    : `Møtet starter ${horizon} (${whenShort}).`;
  return `
    <p style="margin:0 0 14px;">Hei ${firstName},</p>
    <p style="margin:0 0 14px;">Kort påminnelse: ${isInPerson(client) ? 'møtet' : 'online-møtet'} vårt starter ${horizon}.</p>
    <p style="margin:0 0 8px;">${extra}</p>`;
}
...
function withLayout(client, calendar, options, view) {
  ...
  const payload = {
    ...
    signerHtml: signerHtml({ mergeTags }),
...
export function buildSalesEmailPreviewPage(kind = 'thank-you', options = {}) {
  const client = options.client || getSalesEmailPreviewClient();
  ...
  if (!view || view === 'index' || view === 'both') {
    const base = sanitize(options.previewBasePath) || '/email/preview/sales-thank-you';
    return renderSalesEmailPreviewIndex({
      pcHref: `${base}?view=pc`,
      phoneHref: `${base}?view=phone`,
    });
  }
  const previewVariant = view === 'phone' || view === 'mobile' ? 'mobile' : 'desktop';
  const message = kind === 'reminder'
    ? buildSalesReminderEmail(client, calendar, options.reminderKind || '24h', {
      assetBase: options.assetBase,
  ...
  const note = escapeHtml(
    previewVariant === 'mobile'
      ? 'Telefon-versjon av malen. Google Calendar-feltet i Gmail kommer i tillegg når invitasjonen sendes.'
      : 'PC-versjon av malen. Google Calendar-feltet i Gmail kommer i tillegg når invitasjonen sendes.'
  );
...
export function buildSalesThankYouEmail(client, calendar = {}, options = {}) {
  ...
  const layout = withLayout(client, calendar, options, {
    ...
    mobileTitle: 'Møtet er bekreftet',
...
export function buildSalesReminderEmail(client, calendar = {}, reminderKind = '24h', options = {}) {
  const mergeTags = Boolean(options?.mergeTags);
  const inPerson = isInPerson(client);
  const kind = reminderKind === '1h' ? '1h' : '24h';
  const horizon = kind === '1h' ? 'om 1 time' : 'om 24 timer';
  const subject = inPerson
  ...
  const layout = withLayout(client, calendar, options, {
    ...
    title: kind === '1h' ? 'Møtet starter om 1 time' : 'Påminnelse om møtet',
    ...
    ctaLabel: cta.label,
    ctaUrl: mergeTags ? '{{meetLink}}' : cta.url,
...
export function buildOnlineCalendarInvite(client, calendar = {}, options = {}) {
  ...
  const organizerEmail = resolveOrganizerEmail(client, options?.organizerEmail || 'kontakt@asoldi.com');
...
export function embedInlineEmailAssets(html = '') {
  ...
  for (const spec of Object.values(ASSET_FILES)) {
    ...
    const pattern = new RegExp(`(?:https?:\\/\\/[^"'\\s]+)?\\/email\\/sales\\/${file}`, 'g');
    next = next.replace(pattern, `cid:${spec.cid}`);
  }
  return {
    html: next,
    attachments: buildSalesEmailInlineAttachments(),
  };
}
