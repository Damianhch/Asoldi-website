...
function buildOnlineJoinLine(calendar = {}) {
  ...
  return 'Møtelenke ligger i kalenderinvitasjonen.';
}
...
function buildOnlineCalendarInvite(client, calendar = {}, options = {}) {
  ...
  const description = [
    ...
    sanitize(calendar?.meetLink) ? `Google Meet: ${sanitize(calendar.meetLink)}` : '',
  ...
  const icsLines = [
    ...
    `LOCATION:${escapeIcsText(sanitize(calendar?.meetLink) || 'Google Meet')}`,
