...
export function refreshOfferShell(html = '') {
  return foldContractSentence(unboldOfferTermsLabel(String(html || '')
...
export function buildOfferBodyHtml({ products = [], nuances = {}, mvaIncluded = false } = {}) {
  ...
  const benefitsHtml = paragraphs(nuances.benefits)
    || p(placeholder('Hva dere la vekt på at dere vil oppnå, som vi kommer til å fokusere på'));
  ...
  return `
    ...
    <p style="margin:0 0 14px;">Vi tar en workshop der vi går gjennom dette og spør etter det som eventuelt mangler i tillegg.</p>
    ${h2('Hva som skjer fremover')}
    <p style="margin:0 0 14px;">Vedlagt ligger kontrakten for pakken. Den signeres først når dere har bestemt dere, og ingenting betales før nettsiden er levert. Når den er på plass avtaler vi oppstart og gir deg tilgang så du kan følge fremgangen. Ta gjerne kontakt om noe i tilbudet skal justeres.</p>
