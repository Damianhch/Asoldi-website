...
export function ensurePlatformParagraph(html = '') {
  const source = String(html || '');
  const paragraph = `<p data-offer-fixed="platform" style="margin:0 0 14px;">${OFFER_PLATFORM}</p>`;
  const without = source.replace(/<p\b[^>]*data-offer-fixed="platform"[\s\S]*?<\/p>\s*/i, '')
    .replace(/<p\b[^>]*>[^<]*eget CMS[\s\S]*?<\/p>\s*/i, '');
  const delivery = without.search(/data-offer-slot="delivery"/);
  if (delivery >= 0) {
    const close = without.indexOf('</p>', delivery);
    if (close >= 0) {
      const at = close + 4;
      return `${without.slice(0, at)}${paragraph}${without.slice(at)}`;
    }
  }
  return `${without}${paragraph}`;
}
...
export function buildOfferBodyHtml({ products = [], nuances = {}, mvaIncluded = false } = {}) {
  ...
  return `
    ...
    <p data-offer-fixed="platform" style="margin:0 0 14px;">${OFFER_PLATFORM}</p>
