...
export function unboldOfferTermsLabel(html = '') {
  return String(html || '').replace(/<strong>\s*Kundebetingelser:\s*<\/strong>/gi, 'Kundebetingelser:');
}

export function refreshOfferShell(html = '') {
  return unboldOfferTermsLabel(String(html || '')
...
export function fillOfferSlots(html = '', { need = '', project = [], terms = '', benefits = '' } = {}, { onlyOpen = false } = {}) {
  ...
  if (text(terms) && open('terms')) {
    next = replaceOfferSlot(next, 'terms', p(`Kundebetingelser: ${escapeHtml(text(terms))}`));
  }
...
export function buildOfferBodyHtml({ products = [], nuances = {}, mvaIncluded = false } = {}) {
  ...
  const termsHtml = text(nuances.terms)
    ? p(`Kundebetingelser: ${escapeHtml(text(nuances.terms))}`)
    : p(`Kundebetingelser: ${placeholder('Hva vi trenger av informasjon fra dere, og hvor mye dere ønsker å påvirke underveis i prosjektet')}`);
