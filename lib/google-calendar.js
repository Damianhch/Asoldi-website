...
export function buildMeetingAttendees(client, { includeAttendees = true } = {}) {
  if (!includeAttendees) return [];
...
export async function upsertMeetingEvent(client, existingEventId = '', accountKey, options = {}) {
  ...
  const attendees = buildMeetingAttendees(client, { includeAttendees });
  const attendeeEmail = sanitizeText(client?.contactEmail);
  const forceConference = Boolean(options?.forceConference);
  const eventId = sanitizeText(existingEventId || client?.calendar?.eventId);
  const existingMeetLink = sanitizeText(client?.calendar?.meetLink);
  const needsConference =
    isOnline
    && (forceConference || !eventId || !isRealGoogleMeetLink(existingMeetLink));
  const eventBody = {
  ...
  if (forceGuestInvite && eventId && attendeeEmail) {
    try {
      const existing = await calendar.events.get({ calendarId, eventId });
      const currentAttendees = Array.isArray(existing?.data?.attendees) ? existing.data.attendees : [];
      const guest = attendeeEmail.toLowerCase();
      const alreadyListed = currentAttendees.some(
        (entry) => sanitizeText(entry?.email).toLowerCase() === guest
      );
      if (alreadyListed) {
  ...
  let meetLink = sanitizeText(extractMeetLink(event));
