...
/** Match a pasted Fireflies meeting title (or id) against meetings the webhook already stored. */
export function rankMeetingsByTitle(meetings = [], query = '') {
  ...
  return (Array.isArray(meetings) ? meetings : [])
    .map((row) => {
      ...
      let score = 0;
      if (id && id === needle) score = 100;
      else if (title && title === needle) score = 90;
      else if (title && title.includes(needle)) score = 70;
      else if (title && needle.includes(title) && title.length >= 8) score = 50;
