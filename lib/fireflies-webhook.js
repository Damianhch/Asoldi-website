...
export function storeFirefliesMeeting(record = {}) {
  ...
  if (!id) return;
  const map = pruneMap(readJsonMap(MEETINGS_PATH));
...
export async function notifyFirefliesRecording(
  ...
  if (wasFirefliesMeetingNotified(meetingId)) {
    const stored = readStoredFirefliesMeeting(meetingId) || { meetingId, event };
    return { sent: false, reason: 'already-notified', ...publicMeetingFields(stored) };
  }
