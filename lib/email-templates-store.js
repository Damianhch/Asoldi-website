...
import { applyMerge, stripRetiredEmailPhrases } from './email-merge.js';
...
function normalizeTemplate(raw = {}) {
  ...
  return {
    ...
    html: stripRetiredEmailPhrases(String(raw.html || '')),
...
function generatedEmail(client, templateKey) {
  const calendar = client?.calendar || {};
  if (templateKey === 'reminder-1h') return buildSalesReminderEmail(client, calendar, '1h', { embed: true });
  if (templateKey === 'reminder-24h' || templateKey === 'reminder') {
    return buildSalesReminderEmail(client, calendar, '24h', { embed: true });
  }
  return buildSalesThankYouEmail(client, calendar, { embed: true });
}
...
export function composeEmailForClient(client, templateKey = 'thank-you', composed = null) {
  const key = sanitizeText(templateKey) || 'thank-you';
  const generated = generatedEmail(client, key);
  const template = getEmailTemplateById(key);
  const values = salesEmailMergeMap(client, client?.calendar || {});
  const htmlRaw = stripRetiredEmailPhrases(String(composed?.html || template?.html || generated.html || ''));
  ...
  const withPreheader = preheader && !html.includes('display:none')
    ...
    : html;
  const embedded = embedInlineEmailAssets(withPreheader);
  return {
    template,
    generated,
    merged: {
