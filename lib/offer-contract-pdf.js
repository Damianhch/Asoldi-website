...
function scopeForTier(tier) {
  return {
    ...
    priceLine: `Monthly price: ${formatKr(tier.monthlyExMva)} excl. VAT (${formatKr(withMva(tier.monthlyExMva))} incl. VAT), billed monthly.`,
...
function scopeForSummary(summary = {}) {
  const products = Array.isArray(summary.products) ? summary.products : [];
  const exMva = Number(summary.monthlyExMva) || products.reduce((sum, item) => sum + (Number(item.priceExMva) || 0), 0);
  const blocks = products.map((item) => ({
    lead: `${text(item.name) || 'Product'}${Number(item.pages) ? ` – up to ${item.pages} main pages` : ''}${Number(item.priceExMva) ? ` (${formatKr(item.priceExMva)} excl. VAT / month)` : ''}`,
  ...
  return {
    ...
    priceLine: `Monthly price (total): ${formatKr(exMva)} excl. VAT (${formatKr(withMva(exMva))} incl. VAT), billed monthly.`,
    blocks,
    deliveryWeeks: weeks,
 ...
 */
function resolveScope({ tierId = '', summary = null, preferSummary = false } = {}) {
  const tier = tierId && tierId !== CUSTOM_TIER_ID ? tierById(tierId) : null;
  if (summary && (preferSummary || !tier)) {
    const scope = scopeForSummary(summary);
    if (tier) {
  ...
  if (tier) return scopeForTier(tier);
 ...
 */
export async function buildContractPdf({ client = {}, tierId = '', summary = null, preferSummary = false, date = new Date(), blank = false } = {}) {
  const scope = resolveScope({ tierId, summary, preferSummary });
  ...
  doc.font('Helvetica').text('Signature:', MARGIN, sigY + 22, { width: colWidth });
  doc.moveTo(MARGIN, sigY + 58).lineTo(MARGIN + colWidth, sigY + 58).lineWidth(0.8).stroke('#222222');
  doc.text(`Date: ${blank ? '____________' : formatDateNo(date)}`, MARGIN, sigY + 66, { width: colWidth });

  const rightX = MARGIN + colWidth + 30;
  doc.font('Helvetica-Bold').text(`For Client (${partyName}):`, rightX, sigY, { width: colWidth });
  doc.font('Helvetica').text(`Signature (${partyOwner}):`, rightX, sigY + 22, { width: colWidth });
  doc.moveTo(rightX, sigY + 58).lineTo(rightX + colWidth, sigY + 58).lineWidth(0.8).stroke('#222222');
  doc.text('Date: ____________', rightX, sigY + 66, { width: colWidth });
