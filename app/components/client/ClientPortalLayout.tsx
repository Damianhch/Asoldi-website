...
import { Gift, MessageSquare, Search, UserCircle2, ChevronRight, LogOut } from 'lucide-react';
          ...
          <nav className="space-y-2">
            ...
            {isServices ? (
              <div className="ml-3 mt-2 space-y-1 border-l border-[#E5E7EB] pl-3">
                ...
                <span className="block rounded-md px-2 py-1.5 text-xs text-[#9CA3AF]">Email (låst)</span>
          ...
          <header className="h-[72px] bg-white border-b border-[#E7E9EE] px-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative group">
                <button type="button" className="rounded-full border border-[#E5E7EB] p-2 bg-white" aria-label="Bruker">
                  <UserCircle2 size={18} />
                </button>
                <div className="absolute right-0 mt-2 hidden min-w-[220px] rounded-xl border border-[#E5E7EB] bg-white p-3 shadow-lg group-hover:block">
                  <p className="text-sm font-medium">{profile?.name || 'Kunde'}</p>
                  <p className="text-xs text-[#6B7280]">{profile?.email || ''}</p>
                  <button
                    type="button"
                    onClick={logout}
                    className="mt-3 inline-flex w-full items-center gap-2 rounded-lg border border-[#F3D2C0] px-3 py-2 text-sm text-[#B45309] hover:bg-[#FFF7ED]"
                  >
                    <LogOut size={14} />
                    Logg ut
                  </button>
